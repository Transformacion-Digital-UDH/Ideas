    ...
    <x-dialog-modal wire:model="showModal" maxWidth="md">
        ...
        <x-slot name="content">
            ...
            <div class="mt-6">
                ...
                <x-input id="email" class="block mt-1 w-full" type="email" autocomplete="off" wire:model="email" />
                <x-input-error for="email" class="mt-2" />
            </div>

            <div class="mt-6">
                <x-label for="password" value="{{ __('Password') }}" />
                <x-input id="password" class="block mt-1 w-full" type="password" autocomplete="off" wire:model="password" />
                <x-input-error for="password" class="mt-2" />
            </div>

            <div class="mt-6">
                <x-label for="telefono" value="{{ __('Teléfono') }}" />
                <x-input id="telefono" class="block mt-1 w-full" type="text" autocomplete="off" wire:model="telefono" />
                <x-input-error for="telefono" class="mt-2" />
            </div>

            <div class="mt-6">
                <x-label for="profesion" value="{{ __('Profesión') }}" />
                <x-input id="profesion" class="block mt-1 w-full" type="text" autocomplete="off" wire:model="profesion" />
                <x-input-error for="profesion" class="mt-2" />
            </div>

            <div class="mt-6">
                <x-label for="descripcion" value="{{ __('Más detalles') }}" />
                <x-textarea id="descripcion" class="block mt-1 w-full" type="text" autocomplete="off" wire:model="descripcion"
